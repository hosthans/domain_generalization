...
def visualize_models(models, test_loader, target_class=0, save_path=None, max_samples=1000):
    ...
    for idx, (name, model) in enumerate(models.items()):
        ...
        features, labels = extract_features(model, test_loader, max_samples=max_samples)
        
        # adjust t-SNE parameters based on dataset size
        perplexity = min(30, len(features)-1, 50)
        n_iter = 1000 if len(features) > 500 else 500
        
        embedding = TSNE(n_components=2, perplexity=perplexity, max_iter=n_iter, random_state=42).fit_transform(features)
...
def visualize_layer(model, test_loader, layer_name, target_class=0):
    ...
    features, labels = extract_layer_features(model, test_loader, layer_name, max_samples=1000)
    ...
    try:
        score = silhouette_score(embedding, labels)
        plt.title(f"{layer_name} - Silhouette: {score:.3f} ({len(features)} samples)")
    except:
...
def track_feature_evolution(model, test_loader, epoch, target_domain, 
    ...
    if results_tracker is None:
        results_tracker = {'epochs': [], 'silhouette_scores': [], 'target_domain': target_domain}
    ...
    if should_visualize:
        ...
        class_names = [cls for i, cls in enumerate(PACS_CLASSES) if i != target_domain] if isinstance(target_domain, int) else PACS_CLASSES
        unique_labels = np.unique(labels)
        colors = plt.cm.Set3(np.linspace(0, 1, len(unique_labels)))
        ...
        plt.suptitle(f"Feature Analysis - Target: {target_domain} ({len(features)} samples)", fontsize=14)
        plt.tight_layout()
        
        # Save if requested
        if save_dir:
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"epoch_{epoch:02d}_{target_domain}.png")
...
def plot_feature_evolution(results_tracker):
    ...
    plt.plot(results_tracker['epochs'], results_tracker['silhouette_scores'], 'b-o', linewidth=2, markersize=6)
    plt.xlabel('Epoch')
    plt.ylabel('Silhouette Score')
    plt.title(f'Feature Separability Evolution - Target: {results_tracker["target_domain"]}')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show() 